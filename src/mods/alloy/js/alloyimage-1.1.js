...
(function(h) {
    ...
    window[h] = function(b, a, c) {
        if (this instanceof window[h]) {
            ...
            isNaN(b) ? (f.width = parseInt(b.width), f.height = parseInt(b.height), a = window.getComputedStyle(b), imgWidth = parseInt(a.getPropertyValue("width")), imgHeight = parseInt(a.getPropertyValue("height")), isNaN(imgWidth) ? g.drawImage(b, 0, 0) : g.drawImage(b, 
    ...
    {act: function(b, a) {
            ...
            for (var f = 0; f < this.layers.length; f++) {
                var g = this.layers[f], e = g[0].layers, j = g[0];
        ...
        clone: function() {
            ...
            if (!this.layers.length)
                return this.context.putImageData(this.imgData, 0, 0), this.canvas.toDataURL();
            var a = new window[h](this.canvas.width, 
            this.canvas.height);
            a.add(this, "\u6b63\u5e38", 0, 0, b);
            this.tempPsLib = a;
            for (var c = 0; c < this.layers.length; c++) {
                var f = this.layers[c], g = f[0].layers, e = f[0];
            ...
            if (!(b = document.getElementById("imgRect"))){
                ...
                if (typeof FlashCanvas != "undefined") {
                 ...
                 b.id = "imgRect", document.body.appendChild(b), 
            ...
            for (var c = [], f = this.tempPsLib.imgData.data, g = 0, e = f.length; g < e; g++)
                c[f[g]] ? c[f[g]]++ : c[f[g]] = 1;
            a.beginPath();
            a.moveTo(0, b.height);
            for (g = f = 0; 255 > g; g++)
                c[g] > f && (f = c[g]);
            for (g = 0; 255 > g; g++)
                e = c[g] || 0, e = b.height - 0.8 * (e / f) * b.height, a.lineTo(g / 256 * b.width, e, 1, 1);
...
(function(h) {
    window[h].module("addLayer", function() {
        return {add: function(q, b, a, c, f, g, e, j) {
                ...
                j = j || "RGB";
                /[RGB]+/.test(j) || (j = "RGB");
                var n = j.replace("R", "0").replace("G", "1").replace("B", "2"), k;
                j = q.width;
                var h = l.length;
                b = b.width;
    ...
    window[h].module("applyMatrix", function(q) {
        return {process: function(b) {
                for (var a = b.data, c = b.width, f = new q.lib.dorsyMath.Matrix([-2, -4, -4, -4, -2, -4, 0, 8, 0, -4, -4, 8, 24, 8, -4, -4, 0, 8, 0, -4, -2, -4, -4, -4, -2], 25, 1), g = [], e = 0, j = a.length; e < j; e += 4) {
                    var l = e / 4, n = parseInt(l / c), k = l % c;
                    if (!(0 == n || 0 == k)) {
                        for (var h = [[], [], []], d = -2; 3 > d; d++)
...
(function(h) {
    window[h].module("config", function() {
        var q = {"\u7070\u5ea6\u5904\u7406": "toGray","\u53cd\u8272": "toReverse","\u7070\u5ea6\u9608\u503c": "toThresh","\u9ad8\u65af\u6a21\u7cca": "gaussBlur","\u4eae\u5ea6": "brightness","\u6d6e\u96d5\u6548\u679c": "embossment","\u67e5\u627e\u8fb9\u7f18": "borderline","\u8272\u76f8/\u9971\u548c\u5ea6\u8c03\u8282": "setHSI","\u9a6c\u8d5b\u514b": "mosaic","\u6cb9\u753b": "oilPainting","\u8150\u8680": "corrode","\u9510\u5316": "sharp","\u6dfb\u52a0\u6742\u8272": "noise",
            "\u66f2\u7ebf": "curve","\u6697\u89d2": "darkCorner","\u55b7\u70b9": "dotted"}, b = {"\u7f8e\u80a4": "softenFace","\u7d20\u63cf": "sketch","\u81ea\u7136\u589e\u5f3a": "softEnhancement","\u7d2b\u8c03": "purpleStyle","\u67d4\u7126": "soften","\u590d\u53e4": "vintage","\u9ed1\u767d": "gray","\u4efflomo": "lomo","\u4eae\u767d\u589e\u5f3a": "strongEnhancement","\u7070\u767d": "strongGray","\u7070\u8272": "lightGray","\u6696\u79cb": "warmAutumn","\u6728\u96d5": "carveStyle","\u7c97\u7cd9": "rough"};
        return {getModuleName: function(a) {
                return q[a] || 
                a
    ...
    window[h].module("corrode", function() {
        return {process: function(q, b) {
                for (var a = parseInt(b[0]) || 3, c = q.data, f = q.width, g = q.height, e = 0; e < f; e++)
                    for (var j = 0; j < g; j++)
                        for (var l = parseInt(2 * Math.random() * a) - a, n = parseInt(2 * Math.random() * a) - a, k = j * f + e, l = (j + l) * f + e + n, n = 0; 3 > n; n++)
                            c[4 * k + n] = c[4 * l + n];
                return q
            }}
    })
...
(function(h) {
    window[h].module("dorsyMath", function(q) {
        var b = {FFT1: function(a) {
                function c() {
                    ...
                    for (var j = f / Math.pow(2, g), n = f / j, k = 0; k < j; k++)
                        for (var q = k * n, d = (k + 1) * n - 1, p = g, m = Math.pow(2, p - 1), h = 0; q <= d - m; q++) {
                            var r = q + m, v = h * f / Math.pow(2, p), u = v + f / 4;
            ...
            Matrix: function(a, c, b) {
                ...
                if (c) {
                    ...
                    this.m = a.length, this.n = a[0].length;
                this.data = g
            },C: function(a, c) {
                this.r = a || 0;
                this.i = c || 0
                ...
                for (h = g.length; k < h; k++)
                    l[k] && (g[k] = l[k] < f ? l[k] : g[k]);
                return a
            },RGBToHSI: function(a, c, b) {
                var g = (a - c + a - b) / 2 / Math.sqrt((a - c) * (a - c) + (a - b) * (c - b)) || 0, g = Math.acos(g), g = b > c ? 2 * Math.PI - g : g, e = 1 - 3 * Math.min(a, c, b) / (a + c + b);
                ...
                if (a <= 2 * Math.PI / 3)
                    var g = f * (1 - b), e = f * (1 + b * Math.cos(a) / Math.cos(Math.PI / 3 - a)), j = 3 * f - (e + g);
                else
                    a <= 4 * Math.PI / 3 ? (a -= 
        ...
        b.C.prototype = {plus: function(a) {
                ...
                var c = new b.C, f = a.mutiply(a.conjugated());
        ...
        return {getFun: function(q) {
                return {softenFace: function() {
                    ...
                    lomo: function() {
                        ...
                        return this.act("\u7070\u5ea6\u5904\u7406").act("\u66f2\u7ebf", 
                        [0, 60, 142, 194, 255], [0, 194, 240, 247, 255])
                    },warmAutumn: function() {
    ...
    window[h].module("embossment", function() {
        return {process: function(q) {
                for (var b = q.data, a = q.width, c = [], f = 0, g = b.length; f < g; f += 4) {
                    var e = f / 4, j = parseInt(e / a), l = e % a, e = 4 * ((j - 1) * a + (l - 1)), n = 4 * (j + 1) * a + 4 * (l + 1);
                    if (!(0 == j || 0 == l)) {
                        for (j = 0; 3 > j; j++)
                            c[f + j] = b[e + j] - b[n + j] + 127.5;
                        c[f + 4] = b[f + 4]
                    }
                }
                f = 0;
...
(function(h) {
    window[h].module("gaussBlur", function() {
        return {process: function(q, b, a) {
                var c = q.data, f = q.width, g = q.height, e = [], j = 0, l, n, k, h, d, p;
                b = Math.floor(b) || 3;
                a = a || b / 3;
                ...
                for (l = 0; l < g; l++)
                    for (a = 0; a < f; a++) {
                        j = n = k = h = 0;
                        for (p = -b; p <= b; p++)
                            d = a + p, 0 <= d && d < f && (d = 4 * (l * f + d), n += c[d] * e[p + b], k += c[d + 1] * e[p + b], h += c[d + 2] * e[p + b], j += e[p + b]);
                        d = 4 * (l * f + a);
                        c[d] = n / j;
                        c[d + 1] = k / j;
                        c[d + 2] = h / j
                ...
                0; a < f; a++)
                    for (l = 0; l < g; l++) {
                        j = n = k = h = 0;
                        for (p = -b; p <= b; p++)
                            d = l + p, 0 <= d && d < g && (d = 4 * (d * f + a), n += c[d] * e[p + b], k += c[d + 1] * e[p + b], h += c[d + 2] * e[p + b], j += e[p + b]);
                        d = 4 * (l * f + a);
                        c[d] = n / j;
                        c[d + 1] = k / j;
                        c[d + 2] = h / j
        ...
        return {process: function(h, b) {
                for (var a = parseInt(b[0]) || 3, c = h.data, f = h.width, g = h.height, a = 2 * a + 1, e = 0, j = parseInt(f / a); e < j; e++)
                    for (var l = 0, n = parseInt(g / a); l < n; l++) {
                        ...
                        for (d = 0; d < a; d++)
                            for (p = 0; p < a; p++)
                                m = (l * a + d) * f + e * a + p, c[4 * m] = k[0], c[4 * m + 1] = k[1], c[4 * m + 2] = k[2]
                    }
                return h
            }}
    })
    ...
    window[h].module("sharp", function() {
        return {process: function(h, b) {
                for (var a = b[0] || 0.6, c = h.data, f = h.width, g = 0, e = c.length; g < e; g += 4) {
                    var j = g / 4, l = parseInt(j / f), n = j % f;
                    if (!(0 == l || 0 == n))
                        for (var k = 4 * ((l - 1) * f + (n - 1)), l = 4 * ((l - 1) * f + n), j = 4 * (j - 1), n = 0; 3 > n; n++)
...
(function(h) {
    window[h].module("toReverse", function() {
        return {process: function(h) {
                for (var b = h.data, a = 0, c = b.length; a < c; a += 4)
                    b[a] = 255 - b[a], b[a + 1] = 255 - b[a + 1], b[a + 2] = 255 - b[a + 2];
                h.data = b;
                return h
            }}
    })
